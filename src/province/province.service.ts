...
export class ProvinceService {
  ...
  async update(dto: UpdateProvinceDto, userId: number): Promise<any> {
    const provinceModel = await this.provinceRepository.findOne({
      id: dto.id,
    });
    if (!provinceModel) {
    ...
    const em = this.em.fork();
    try {
      provinceModel.name = dto.name;
      provinceModel.updatedBy = userId;
      await this.em.flush();
  ...
  async remove(id: number, userId: number): Promise<any> {
    ...
    try {
      await provinceModel.softDelete(this.em, userId);
      return true;
    } catch (error) {}
  }
}
